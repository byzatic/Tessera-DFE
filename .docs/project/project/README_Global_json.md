...
| `cronMetricUpdateString` | Cron выражение для запуска обработки данных storage              |
...
Все глобальные `storages` автоматически доступны в каждом nodes pipeline но не перекрывают их:
